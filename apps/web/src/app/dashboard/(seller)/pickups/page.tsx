...
const STATUS_BADGE: Record<string, { bg: string; text: string; border: string; label: string }> = {
  scheduled: { bg: "bg-[#EDF0E4]", text: "text-[#546B41]", border: "border-[#CBD7B5]", label: "Scheduled" },
  picked_up: { bg: "bg-[#EDF0E4]", text: "text-[#546B41]", border: "border-[#CBD7B5]", label: "Picked Up" },
  failed: { bg: "bg-[#FEF2F2]", text: "text-[#A84A3B]", border: "border-[#FECACA]", label: "Failed" },
  cancelled: { bg: "bg-[#F8F9F7]", text: "text-[#8A9270]", border: "border-[#EADFC8]", label: "Cancelled" },
};
...
export default function PickupsPage() {
  ...
  const pickupCards = [
    ...
    },
    {
      ...
      value: pickups.filter((p) => p.status === "cancelled" || p.status === "failed").length,
      color: "#A84A3B",
      iconBg: "#FEF2F2",
      icon: "⚠️",
    },
  ...
  const filteredPickups = pickups.filter((p) => {
    ...
    if (activeFilter === "cancelled_failed") return p.status === "cancelled" || p.status === "failed";
    return true;
  });
                ...
                <tr>
                  <td colSpan={6} className="px-5 py-16 text-center">
                    ...
                    <p className="text-sm font-bold text-[#2F3A22]">
                      No {activeFilter === "all" ? "" : activeFilter.replace("_", " ")} pickups found.
                    </p>
                  </td>
                </tr>
              ) : (
      ...
      {isModalOpen && mounted && createPortal(
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md animate-fade-up border border-[#EADFC8] flex flex-col max-h-[90vh] overflow-hidden">
              ...
              <div className="flex items-start justify-between">
                <div className="flex gap-3 items-center">
                  <div className="w-10 h-10 rounded-xl bg-[#546B41] flex items-center justify-center text-white shrink-0 shadow-sm">
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                  </div>
                  <div>
            ...
            <form className="overflow-y-auto flex-1 min-h-0" onSubmit={handleCreate}>
