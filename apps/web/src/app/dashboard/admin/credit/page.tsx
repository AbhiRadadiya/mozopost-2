...
export default function AdminCreditPage() {
  ...
  async function handleRecover(e: React.FormEvent) {
    ...
    try {
      const { data } = await api.post(
        `/admin/credit/${recoverFacilityId}/recover-from-cod`,
        { codSettlementAmount: parseFloat(codAmount) },
      );
      setMessage(
        `Recovered ₹${data.recovered.toFixed(2)} · Released ₹${data.releasedToMerchant.toFixed(2)} to merchant`,
      );
      setCodAmount("");
      setRecoverFacilityId("");
      load();
    } catch (err) {
      setError(apiErrorMessage(err));
    } finally {
  ...
  const utilizationPct =
    stats && stats.total_credit_limit > 0
      ? Math.round((stats.total_outstanding / stats.total_credit_limit) * 100)
      : 0;
    ...
    <div className="animate-fade-up  mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#0F172A]">
          Credit / Postpaid Wallet Management
        </h1>
        <p className="text-sm text-[#64748B] mt-1">
          Manage merchant credit facilities and outstanding balances.
        </p>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-white p-5 rounded-2xl border border-[#E5E8EF] shadow-sm">
            ...
            <div className="text-2xl font-bold text-[#0F172A] font-mono">
              ₹{stats.total_credit_limit.toFixed(0)}
            </div>
          </div>
          <div className="bg-[#FEF2F2] p-5 rounded-2xl border border-[#FECACA] shadow-sm">
            ...
            <div className="text-2xl font-bold text-[#DC2626] font-mono">
              ₹{stats.total_outstanding.toFixed(0)}
            </div>
          </div>
          <div className="bg-[#F0FDF4] p-5 rounded-2xl border border-[#A7F3D0] shadow-sm">
            ...
            <div className="text-2xl font-bold text-[#16A34A] font-mono">
              ₹{stats.total_available.toFixed(0)}
            </div>
          </div>
          <div
            className={`p-5 rounded-2xl border shadow-sm ${utilizationPct >= 80 ? "bg-[#FEF2F2] border-[#FECACA]" : "bg-[#FEF9C3] border-[#FEF08A]"}`}
          >
            <div
              className={`text-[10px] font-bold uppercase tracking-widest mb-2 ${utilizationPct >= 80 ? "text-[#991B1B]" : "text-[#854D0E]"}`}
            >
              Platform Utilization
            </div>
            <div
              className={`text-2xl font-bold font-mono ${utilizationPct >= 80 ? "text-[#DC2626]" : "text-[#CA8A04]"}`}
            >
              {utilizationPct}%
            </div>
          </div>
          <div className="bg-white p-5 rounded-2xl border border-[#E5E8EF] shadow-sm">
            ...
            <div className="text-2xl font-bold text-[#4F46E5] font-mono">
              {stats.total_merchants_with_credit}
            </div>
          </div>
          <div className="bg-white p-5 rounded-2xl border border-[#E5E8EF] shadow-sm">
            <div className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest mb-2">
      ...
      {parseInt(stats?.near_limit_count || "0") > 0 && (
        <div className="p-4 rounded-xl bg-[#FEF9C3] border border-[#FEF08A] text-sm font-semibold text-[#854D0E] flex items-center gap-2">
          ⚠ {stats?.near_limit_count} merchant(s) have used &gt;80% of their
          credit limit
        </div>
      )}
          ...
          <form onSubmit={handleAssign} className="p-6 space-y-4">
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setAssignForm((p) => ({ ...p, sellerId: e.target.value }))
                }
            ...
            <div className="flex items-center justify-between p-3.5 rounded-xl bg-[#F4F6F9] border border-[#E5E8EF]">
              ...
              <button
                ...
                onClick={() =>
                  setAssignForm((p) => ({
                    ...p,
                    autoBlockAtLimit: !p.autoBlockAtLimit,
                  }))
                }
              ...
              >
                <span
                  className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-all shadow ${assignForm.autoBlockAtLimit ? "left-4" : "left-0.5"}`}
                />
              </button>
            </div>
            <button
        ...
        <div className="bg-white rounded-2xl border border-[#E5E8EF] shadow-sm overflow-hidden">
          ...
          <form onSubmit={handleRecover} className="p-6 space-y-4">
            <div className="p-4 rounded-xl bg-[#EEF2FF] border border-[#C7D2FE] text-xs text-[#4338CA]">
              ...
              When a COD settlement arrives, the system first recovers the
              outstanding credit balance, then releases the remainder to the
              merchant's wallet.
              <div className="font-mono mt-2 bg-[#E0E7FF] px-2 py-1 rounded text-[11px]">
            ...
            <div>
              ...
              <select
                required
                value={recoverFacilityId}
                onChange={(e) => setRecoverFacilityId(e.target.value)}
                className="w-full px-3 py-2.5 text-sm border border-[#E5E8EF] rounded-xl bg-white text-[#0F172A] outline-none focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/10"
              >
                <option value="">Select merchant...</option>
                {facilities
                  .filter((f) => parseFloat(f.credit_outstanding) > 0)
                  .map((f) => (
                    <option key={f.id} value={f.id}>
                      {f.business_name} — outstanding ₹
                      {parseFloat(f.credit_outstanding).toFixed(2)}
                    </option>
                  ))}
              </select>
            </div>
            <div>
              ...
              <input
                ...
                placeholder="Total COD received"
        ...
        <div className="bg-white rounded-2xl border border-[#E5E8EF] shadow-sm overflow-hidden">
          {loading ? (
            ...
            <div className="py-16 text-center">
              ...
              <div className="text-sm font-semibold text-[#0F172A]">
                No credit facilities assigned yet
              </div>
              <div className="text-xs text-[#94A3B8] mt-1">
                Use the form above to assign credit to a merchant.
              </div>
            </div>
          ) : (
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-[#F1F5F9]">
                  {facilities.map((f) => {
                    ...
                    const isExhausted =
                      parseFloat(f.credit_outstanding) >=
                      parseFloat(f.credit_limit);
                    const isNear = utilPct >= 80 && !isExhausted;
                    return (
                      <tr
                        key={f.id}
                        className={`hover:bg-[#F8F9FB] transition-colors ${isExhausted ? "bg-[#FFF7F7]" : isNear ? "bg-[#FFFBEB]" : ""}`}
                      >
                        ...
                        <td className="px-4 py-3 text-sm font-mono text-[#0F172A]">
                          ₹{parseFloat(f.wallet_balance).toFixed(0)}
                        </td>
                        <td className="px-4 py-3 text-sm font-mono text-[#0F172A]">
                          ₹{parseFloat(f.credit_limit).toFixed(0)}
                        </td>
                        <td className="px-4 py-3 text-sm font-bold font-mono text-[#DC2626]">
                          ₹{parseFloat(f.credit_outstanding).toFixed(0)}
                        </td>
                        <td className="px-4 py-3 text-sm font-bold font-mono text-[#16A34A]">
                          ₹{parseFloat(f.available_credit).toFixed(0)}
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            <div className="w-16 h-1.5 bg-[#F1F5F9] rounded-full overflow-hidden">
                              <div
                                className={`h-full rounded-full ${isExhausted ? "bg-[#DC2626]" : isNear ? "bg-[#CA8A04]" : "bg-[#16A34A]"}`}
                                style={{ width: `${Math.min(100, utilPct)}%` }}
                              />
                            </div>
                            <span
                              className={`text-xs font-bold ${isExhausted ? "text-[#DC2626]" : isNear ? "text-[#CA8A04]" : "text-[#16A34A]"}`}
                            >
                              {utilPct.toFixed(1)}%
                            </span>
                          </div>
                        ...
                        <td className="px-4 py-3">
                          <span
                            className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${STATUS_STYLE[f.status] || "bg-[#F1F5F9] text-[#475569]"}`}
                          >
                            {f.status}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex gap-2">
                            {f.status === "active" && (
                              <button
                                onClick={() => changeStatus(f.id, "freeze")}
                                className="px-2.5 py-1.5 text-xs font-semibold bg-[#DBEAFE] text-[#1E40AF] rounded-lg hover:bg-[#BFDBFE] transition-colors"
                              >
                                Freeze
                              </button>
                            )}
                            {f.status === "frozen" && (
                              <button
                                onClick={() => changeStatus(f.id, "unfreeze")}
                                className="px-2.5 py-1.5 text-xs font-semibold bg-[#D1FAE5] text-[#065F46] rounded-lg hover:bg-[#A7F3D0] transition-colors"
                              >
                                Unfreeze
                              </button>
                            )}
                            {f.status !== "removed" && (
                              <button
                                onClick={() => changeStatus(f.id, "remove")}
                                className="px-2.5 py-1.5 text-xs font-semibold bg-[#FEE2E2] text-[#991B1B] rounded-lg hover:bg-[#FECACA] transition-colors"
                              >
                                Remove
                              </button>
                            )}
