...
interface Merchant { id: string; business_name: string; email: string; kyc_status: string; wallet_balance: string; }
...
export default function MarginsPage() {
  ...
  const [sellerForm, setSellerForm] = useState({ sellerId: '', courierId: '', marginType: 'fixed', marginValue: '5' });
  ...
  async function handleSetSeller(e: React.FormEvent) {
    e.preventDefault();
    if (!sellerForm.sellerId) { setError('Please select a seller'); return; }
    setSellerSubmitting(true); setError(''); setMessage('');
    try {
    ...
    finally { setSellerSubmitting(false); }
  }

  const globalMargins = margins.filter(m => !m.business_name && !m.courier_name);
  const sellerMargins = margins.filter(m => !!m.business_name);

  const inp = "w-full px-3 py-2.5 text-sm border border-[#E5E8EF] rounded-xl bg-white text-[#0F172A] outline-none focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/10";
  ...
  return (
    <div className="animate-fade-up max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-[#64748B] mt-1">Configure global & seller-wise shipping cost margins.</p>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl border border-[#E5E8EF] shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-[#E5E8EF] bg-gradient-to-r from-[#EEF2FF] to-[#F8F9FB]">
              ...
              <p className="text-xs text-[#64748B] mt-0.5">Applies to all couriers unless overridden per seller.</p>
            </div>
            <form onSubmit={handleSetGlobal} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-[#475569] mb-1.5 uppercase tracking-wide">Value ({marginType === 'fixed' ? '₹' : '%'})</label>
                <input type="number" required value={marginValue} onChange={e => setMarginValue(e.target.value)} className={inp} />
              </div>
          ...
          <div className="bg-white rounded-2xl border border-[#E5E8EF] shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-[#E5E8EF] bg-[#F8F9FB]">
              <h2 className="text-sm font-bold text-[#0F172A]">Active Global Margin Rules</h2>
            </div>
            {loading ? (
              ...
              <div className="max-h-[340px] overflow-y-auto divide-y divide-[#F1F5F9]">
                ...
                {globalMargins.length === 0 && <div className="py-12 text-center text-sm text-[#94A3B8]">No global rules configured.</div>}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-[#E5E8EF] shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-[#E5E8EF] bg-gradient-to-r from-[#F0FDF4] to-[#F8F9FB]">
              <h2 className="text-sm font-bold text-[#0F172A]">Set Seller-wise Margin Rule</h2>
              <p className="text-xs text-[#64748B] mt-0.5">Override global margins for a specific seller or seller+courier combination.</p>
            </div>
            <form onSubmit={handleSetSeller} className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-[#475569] mb-1.5 uppercase tracking-wide">Select Seller *</label>
                  <select required value={sellerForm.sellerId} onChange={e => setSellerForm(p => ({ ...p, sellerId: e.target.value }))} className={inp}>
                    <option value="">— Choose seller —</option>
                    {merchants.map(m => (
                      <option key={m.id} value={m.id}>{m.business_name} ({m.kyc_status})</option>
                    ))}
                  </select>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[#475569] mb-1.5 uppercase tracking-wide">Courier (Optional)</label>
                  <select value={sellerForm.courierId} onChange={e => setSellerForm(p => ({ ...p, courierId: e.target.value }))} className={inp}>
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#475569] mb-1.5 uppercase tracking-wide">Margin Type</label>
                  <div className="flex rounded-xl border border-[#E5E8EF] overflow-hidden">
                    <button type="button" onClick={() => setSellerForm(p => ({ ...p, marginType: 'fixed' }))}
                    ...
                    <button type="button" onClick={() => setSellerForm(p => ({ ...p, marginType: 'percentage' }))}
                ...
                <div>
                  <label className="block text-xs font-semibold text-[#475569] mb-1.5 uppercase tracking-wide">Value ({sellerForm.marginType === 'fixed' ? '₹' : '%'})</label>
                  <input type="number" required value={sellerForm.marginValue} onChange={e => setSellerForm(p => ({ ...p, marginValue: e.target.value }))} className={inp} />
                </div>
              </div>
          ...
          <div className="bg-white rounded-2xl border border-[#E5E8EF] shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-[#E5E8EF] bg-[#F8F9FB]">
              <h2 className="text-sm font-bold text-[#0F172A]">Seller-wise Margin Rules</h2>
              <p className="text-xs text-[#64748B] mt-0.5">KYC status is shown inline for quick verification.</p>
            </div>
            {loading ? (
              <div className="py-12 text-center text-sm text-[#94A3B8] animate-pulse">Loading...</div>
            ) : (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-[#E5E8EF] bg-[#F8F9FB]">
                      <th className="px-5 py-3.5 text-left text-[11px] font-semibold text-[#64748B] uppercase tracking-wider">Seller</th>
                      <th className="px-5 py-3.5 text-left text-[11px] font-semibold text-[#64748B] uppercase tracking-wider">KYC Status</th>
                      <th className="px-5 py-3.5 text-left text-[11px] font-semibold text-[#64748B] uppercase tracking-wider">Courier</th>
                      <th className="px-5 py-3.5 text-left text-[11px] font-semibold text-[#64748B] uppercase tracking-wider">Type</th>
                      <th className="px-5 py-3.5 text-left text-[11px] font-semibold text-[#64748B] uppercase tracking-wider">Value</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-[#F1F5F9]">
                    {sellerMargins.map(m => {
                      const merchant = merchants.find(mer => mer.business_name === m.business_name);
                      const kycStatus = merchant?.kyc_status || 'pending_kyc';
                      return (
                        <tr key={m.id} className="hover:bg-[#F8F9FB] transition-colors">
                          <td className="px-5 py-3.5 text-sm font-semibold text-[#0F172A]">{m.business_name}</td>
                          <td className="px-5 py-3.5">
                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${KYC_STYLE[kycStatus] || 'bg-[#F1F5F9] text-[#475569]'}`}>
                              {kycStatus.replace(/_/g, ' ')}
                            </span>
                          </td>
                          <td className="px-5 py-3.5 text-sm text-[#64748B]">{m.courier_name || 'All Couriers'}</td>
                          <td className="px-5 py-3.5">
                            <span className="px-2 py-0.5 rounded text-xs font-semibold bg-[#EEF2FF] text-[#4F46E5] capitalize">{m.margin_type}</span>
                          </td>
                    ...
                    {sellerMargins.length === 0 && (
                      <tr><td colSpan={5} className="px-5 py-12 text-center text-sm text-[#94A3B8]">No seller-wise margin rules configured yet.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
