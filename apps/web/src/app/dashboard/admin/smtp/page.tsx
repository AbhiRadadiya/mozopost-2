...
export default function SmtpPage() {
  ...
  async function saveConfig(e: React.FormEvent) {
    ...
    setError("");
    try {
      await api.post("/admin/smtp/configs", {
        ...form,
        port: parseInt(form.port),
        secure: form.secure || parseInt(form.port) === 465,
      });
      setShowForm(false);
      load();
    } catch (err) {
  ...
  async function toggleRule(id: string, isActive: boolean) {
    try {
      ...
      setRules((r) =>
        r.map((x) => (x.id === id ? { ...x, is_active: isActive } : x)),
      );
    } catch (err) {
      setError(apiErrorMessage(err));
    }
  ...
  const inp =
    "w-full px-3 py-2.5 text-sm border border-[#E5E8EF] rounded-xl bg-white text-[#0F172A] outline-none focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/10 placeholder:text-[#94A3B8]";
    ...
    <div className="animate-fade-up  mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-[#64748B] mt-1">
          Configure email delivery, templates, and notification rules.
        </p>
      </div>
      ...
      {tab === "configs" && (
        <div className="space-y-4">
          ...
          <div className="bg-[#EEF2FF] border border-[#C7D2FE] rounded-xl p-4 flex items-center gap-3">
            ...
            />
            <span className="text-xs font-semibold text-[#4F46E5] whitespace-nowrap">
              Test recipient
            </span>
          </div>
          ...
          {testResult && (
            <div
              className={`p-4 rounded-xl text-sm font-medium border ${testResult.ok ? "bg-[#F0FDF4] border-[#A7F3D0] text-[#065F46]" : "bg-[#FEF2F2] border-[#FECACA] text-[#991B1B]"}`}
            >
              {testResult.ok ? "✓ " : "✕ "}
              {testResult.message}
            </div>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {configs.map((c) => (
                <div
                  key={c.id}
                  className="bg-white rounded-2xl border border-[#E5E8EF] shadow-sm p-5"
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        {c.is_default && (
                          <span className="px-2 py-0.5 rounded-full text-[11px] font-semibold bg-[#D1FAE5] text-[#065F46]">
                            Default
                          </span>
                        )}
                      </div>
                      <div className="text-xs text-[#64748B]">
                        {c.host}:{c.port} · {c.from_email}
                      </div>
                      {c.last_tested_at && (
                        <div className="text-xs mt-1.5 flex items-center gap-1.5 text-[#94A3B8]">
                          Last tested:{" "}
                          {new Date(c.last_tested_at).toLocaleDateString(
                            "en-IN",
                          )}{" "}
                          —
                          <span
                            className={`px-1.5 py-0.5 rounded text-[10px] font-semibold ${c.test_status === "ok" ? "bg-[#D1FAE5] text-[#065F46]" : "bg-[#FEE2E2] text-[#991B1B]"}`}
                          >
                            {c.test_status}
                          </span>
                        </div>
                    ...
                    <div className="flex flex-col items-end gap-2">
                      <button
                        disabled={testId === c.id}
                        onClick={() => testSmtp(c.id)}
                      ...
                      {testResult?.id === c.id && (
                        <span
                          className={`text-[11px] font-semibold ${testResult?.ok ? "text-[#16A34A]" : "text-[#DC2626]"}`}
                        >
            ...
            templates.map((t) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    <span className="px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-[#DBEAFE] text-[#1E40AF]">
                      {t.event.replace(/_/g, " ")}
                    </span>
                  ...
                  <span
                    className={`px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${t.is_active ? "bg-[#D1FAE5] text-[#065F46]" : "bg-[#F1F5F9] text-[#475569]"}`}
                  >
            ...
            <div className="divide-y divide-[#F1F5F9]">
              {rules.map((r) => (
                ...
                >
                  ...
                  >
                    <span
                      className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-all ${r.is_active ? "left-6" : "left-1"}`}
                    />
                  </button>
                </div>
              ))}
                  ...
                  {logs.length === 0 ? (
                    ...
                    logs.map((l) => (
                      ...
                      >
                        ...
                        <td className="px-5 py-3 text-sm text-[#0F172A] truncate max-w-[200px]">
                          {l.subject}
                        </td>
                        <td className="px-5 py-3">
                          <span
                            className={`px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${LOG_STATUS_STYLE[l.status] || "bg-[#F1F5F9] text-[#475569]"}`}
                          >
                            {l.status}
                          </span>
                        </td>
