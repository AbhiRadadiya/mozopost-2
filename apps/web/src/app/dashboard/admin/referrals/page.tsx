    ...
    <div className="animate-fade-up  mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-[#64748B] mt-1">
          Track and manage referral commissions and payouts.
        </p>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-5 rounded-2xl border border-[#E5E8EF] shadow-sm">
            ...
            <div className="text-2xl font-bold text-[#16A34A] font-mono">
              ₹{parseFloat(stats.total_commission_earned || 0).toFixed(0)}
            </div>
          </div>
          <div className="bg-[#FEF9C3] p-5 rounded-2xl border border-[#FEF08A] shadow-sm">
            ...
            <div className="text-2xl font-bold text-[#CA8A04] font-mono">
              ₹{parseFloat(stats.pending_payout || 0).toFixed(0)}
            </div>
          </div>
          <div className="bg-white p-5 rounded-2xl border border-[#E5E8EF] shadow-sm">
            <div className="text-[10px] font-bold text-[#94A3B8] uppercase tracking-widest mb-2">
              ...
              <tbody className="divide-y divide-[#F1F5F9]">
                {referrals.map((r) => {
                  const pending =
                    ...
                    parseFloat(r.paid_commission || 0);
                  return (
                    <tr
                      key={r.id}
                    ...
                    >
                      ...
                      <td className="px-5 py-3.5 text-sm font-bold font-mono text-[#4F46E5]">
                        {r.referral_code}
                      </td>
                      ...
                      <td className="px-5 py-3.5 text-sm font-bold font-mono text-[#16A34A]">
                        ₹{parseFloat(r.paid_commission || 0).toFixed(2)}
                      </td>
                      <td className="px-5 py-3.5">
                        <span
                          className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold ${r.status === "paid" ? "bg-[#D1FAE5] text-[#065F46]" : r.status === "active" ? "bg-[#DBEAFE] text-[#1E40AF]" : "bg-[#FEF9C3] text-[#854D0E]"}`}
                        >
                          {r.status}
                        </span>
                      </td>
