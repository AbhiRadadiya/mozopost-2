...
interface Merchant { id: string; business_name: string; wallet_balance: string; }
  ...
  return (
    <div className="animate-fade-up max-w-7xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-[#64748B] mt-1">Credit or debit seller wallets and view balances.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-[#E5E8EF] shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-[#E5E8EF] bg-[#F8F9FB]">
            <h2 className="text-sm font-bold text-[#0F172A]">Adjust Wallet Balance</h2>
          </div>
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            <div>
              ...
              >
                {merchants.map(m => (
                  <option key={m.id} value={m.id}>{m.business_name} — ₹{parseFloat(m.wallet_balance || '0').toFixed(0)}</option>
                ))}
              </select>
            </div>
            ...
            {/* Type Toggle */}
            <div>
              <label className="block text-xs font-semibold text-[#475569] mb-1.5 uppercase tracking-wide">Action Type</label>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-[#475569] mb-1.5 uppercase tracking-wide">Amount (₹)</label>
        ...
        <div className="bg-white rounded-2xl border border-[#E5E8EF] shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-[#E5E8EF] bg-[#F8F9FB]">
            <h2 className="text-sm font-bold text-[#0F172A]">All Merchant Wallets</h2>
          </div>
          <div className="max-h-[420px] overflow-y-auto divide-y divide-[#F1F5F9]">
            {merchants.map(m => (
              <div key={m.id} className="flex items-center justify-between px-6 py-3.5 hover:bg-[#F8F9FB] transition-colors">
                <div className="text-sm font-medium text-[#0F172A]">{m.business_name}</div>
                <div className={`text-sm font-bold font-mono ${parseFloat(m.wallet_balance || '0') >= 0 ? 'text-[#16A34A]' : 'text-[#DC2626]'}`}>
