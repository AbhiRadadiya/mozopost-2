  ...
  async function submitUtr(e: React.FormEvent) {
    ...
    try {
      const { data } = await api.patch(
        `/admin/cod-settlements/${selected.id}/utr`,
        utrForm,
      );
    ...
    <div className="animate-fade-up  mx-auto space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-[#0F172A]">
          UTR Entry — COD Settlements
        </h1>
        <p className="text-sm text-[#64748B] mt-1">
          Enter UTR numbers to mark COD settlements as paid.
        </p>
      </div>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-[#F1F5F9]">
                  ...
                  {settlements.map((s) => (
                    <tr
                      key={s.id}
                      className={`hover:bg-[#F8F9FB] transition-colors ${selected?.id === s.id ? "bg-[#EEF2FF]" : ""}`}
                    >
          ...
          {selected ? (
              ...
              <div className="p-4 rounded-xl bg-[#F4F6F9] border border-[#E5E8EF]">
                ...
                <div className="text-xs text-[#64748B] mt-1">
                  ...
                  <span className="font-bold font-mono text-[#16A34A]">
                    ₹{parseFloat(selected.net_amount).toFixed(2)}
                  </span>
                </div>
            ...
            <div className="p-12 text-center">
              ...
              <div className="text-xs text-[#94A3B8] mt-1">
                Select a row from the table to enter UTR details
              </div>
            </div>
          )}
        </div>
